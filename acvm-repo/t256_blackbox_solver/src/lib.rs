...
impl T256BlackboxSolver {
    ...
    fn parse_msm_inputs(
        ...
        for i in (0..points.len()).step_by(2) {
            let point =
                Self::coordinates_to_affine(points[i], points[i + 1])
                    .map_err(|e| BlackBoxResolutionError::Failed(BlackBoxFunc::MultiScalarMul, e.to_string()))?;

            let scalar_low: u128 =
                T256BlackboxSolver::field_to_u128_limb(&scalars_lo[i / 2], BlackBoxFunc::MultiScalarMul)?;

            let scalar_high: u128 =
                T256BlackboxSolver::field_to_u128_limb(&scalars_hi[i / 2], BlackBoxFunc::MultiScalarMul)?;
